...
void GLRenderer::Render(SceneObject& node, ParticleSystem& particles) {
    ...
    for (auto& particle : particles.GetParticles()) {
        // Set the model_matrix_ to reflect the particle's transformations (translation, rotation, scaling, etc.)
        model_matrix_ = glm::mat4();

        // OPTIONAL: You can modify the material here to change with each particle if you want.
        // For example, if you give each particle a random color, you can set the material's
        // attributes here to reflect that color.

        // Pass the model_matrix and other uniforms to the shader
        SetUniforms(shader, *material, node);
        // Draw the particle using the mesh
